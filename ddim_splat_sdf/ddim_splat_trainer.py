...
@dataclass
class DDIMSplat_TrainerConfig(TrainerConfig):
    """Configuration for training regimen"""

    _target: Type = field(default_factory=lambda: DDIMSplat_Trainer)
...
class DDIMSplat_Trainer(Trainer):
    """DDIMSplat_Trainer class
    ...
    def __init__(self, config: DDIMSplat_TrainerConfig, local_rank: int = 0, world_size: int = 1) -> None:
    ...
    def setup(self, test_mode: Literal["test", "val", "inference"] = "val") -> None:
        """Setup the DDIMSplat_Trainer by calling other setup functions.
    ...
    def train(self) -> None:
        ...
        # Measures the total training time.
        with TimeWriter(writer, EventName.TOTAL_TRAIN_TIME):
            ...
            for step in range(self._start_step, self._start_step + num_iterations):
                ...
                while self.training_state == "paused":
                    # Early Stop During Pause
                    if self.stop_training:
                        self._after_train()
                        return
                    time.sleep(0.01)
                # Ensures that only one training operation is performed at a time.
    ...
    def train_iteration(self, step: int) -> TRAIN_INTERATION_OUTPUT:
        ...
        with torch.autocast(device_type=cpu_or_cuda_str, enabled=self.mixed_precision):
            _, loss_dict, metrics_dict, usenewpose = self.pipeline.get_train_loss_dict(step=step)
            loss = functools.reduce(torch.add, loss_dict.values())
        self.grad_scaler.scale(loss).backward()  # type: ignore
        if usenewpose:
            # Save current gradients if using a novel view
            for param in self.pipeline.model.parameters():
                if param.grad is not None:
                    if hasattr(self, 'ddim_splat_grad'):
                        self.ddim_splat_grad[param] = param.grad.clone()
                    else:
                        self.ddim_splat_grad = {}
                        self.ddim_splat_grad[param] = param.grad.clone()
        else:
            # Apply saved gradients if they exist and discount them
            if hasattr(self, 'ddim_splat_grad'):
                for param in self.pipeline.model.parameters():
                    if param in self.ddim_splat_grad:
                        param.grad += self.ddim_splat_grad[param] * 0.01  # Discount factor
                # Optionally clear the stored gradients after applying
                # del self.ddim_splat_grad
            needs_step = [
                group

...
class DDIMSplatfactoModelConfig(SplatfactoModelConfig):
    ...
    ddim_scheduler_path: str = "./ddim/ddim-cecum_t1_a01/scheduler"
    """scheduler for pretrained DDIM model"""
    unet_model_path: str ="./ddim/ddim-cecum_t1_a01/unet"
    """pretrained unet model"""
    timesteps: torch.LongTensor = torch.LongTensor([50])
    ...
    psnr_lambda: float = 0.2
    """weight of psnr loss"""
    noise_scaler = 4
    """scaler for noise that sampled from standard normal distribution"""
    df_img_output_dir: str = None
    """diffusion image output directory"""
    diffusion_lambda = 1e-4
...
class DDIMSplatfactoModel(SplatfactoModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.L_diff = 0
        """Diffusion loss that acts as a prior"""
    ...
    def get_df_outputs(self, camera: Cameras) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        # novel image path
        novel_img_path = "/home/wangzican/renders/cecum_t1_a/novel_img"
        ...
        rgb_output = normalize_rgb(rgb_output.to("cpu"))
        # print(rgb_output[:2])

        # Add noise to model output using DDIM noise scheduler
        noise = torch.randn(rgb_output.shape)
        ...
        # noise image path
        noise_img_path = "/home/wangzican/renders/cecum_t1_a/noise_img"
        # # Save the novel images
        # if self.config.df_img_output_dir is not None:
        ...
        trained_diffusion_model = UNet2DModel.from_pretrained(self.config.unet_model_path)

        noisy_rgb_output = noise_scheduler.add_noise(rgb_output.to("cpu"), noise * self.config.noise_scaler, self.config.timesteps)
        pre_image = noisy_rgb_output

        # noisy image path
        noisy_img_path = "/home/wangzican/renders/cecum_t1_a/noisy_img"
        # Save the novel images
        if self.config.df_img_output_dir is not None:
    ...
    def get_loss_dict(self, outputs, gt_img, metrics_dict=None, use_new_pose = False) -> Dict[str, torch.Tensor]:
        ...
        if use_new_pose == True:
            ...
            # self.L_diff = weight[0] * Ll1 / (self.config.noise_scaler^2) + weight[1] * simloss
            loss_dict = {
                "main_loss": self.config.diffusion_lambda * self.config.ssim_lambda * simloss,
                "scale_reg": scale_reg
            }
        else:
            total_w = 1+self.config.ssim_lambda+self.config.psnr_lambda
            weight = [1/total_w, self.config.ssim_lambda/total_w, self.config.psnr_lambda]
            loss_dict = {
